...
class TestDeck(unittest.TestCase):
    ...
    @patch('src.deck.routes.db')
    def test_record_wrong_answer_success(self, mock_db):
        '''Test the /deck/<deck_id>/record-wrong-answer route for a successful record'''
        deck_id = "test_deck_456"
        user_id = "test_user_123"
        test_front = "test front"
        test_back = "test back"
        test_hint = "test hint"
        test_card_id = "test_card_789"

        mock_card = MagicMock()
        mock_card.val.return_value = {
            'front': test_front,
            'back': test_back,
            'hint': test_hint
        }
        mock_card.key.return_value = test_card_id
        
        mock_query_result = MagicMock()
        mock_query_result.each.return_value = [mock_card]
        mock_db.child.return_value.order_by_child.return_value.equal_to.return_value.get.return_value = mock_query_result
        
        mock_wrong_answer_ref = MagicMock()
        mock_db.child.return_value.child.return_value.child.return_value.child.return_value = mock_wrong_answer_ref

        response = self.app.post(
            f'/deck/{deck_id}/record-wrong-answer',
            data=json.dumps({
                "userId": user_id,
                "front": test_front,
                "back": test_back,
        ...
        assert response.status_code == 200
        response_data = json.loads(response.data)
        assert response_data['message'] == "Wrong answer recorded successfully"

        mock_db.child.assert_any_call("card")
        mock_db.child().order_by_child.assert_called_with("front")
        mock_db.child().order_by_child().equal_to.assert_called_with(test_front)

        mock_db.child.assert_any_call("wrong_answers")
        mock_wrong_answer_ref.push.assert_called_once_with(ANY)

    @patch('src.deck.routes.db')
    def test_record_wrong_answer_missing_fields(self, mock_db):
        '''Test the /deck/<deck_id>/record-wrong-answer route with missing required fields'''
        deck_id = "test_deck_456"
        test_data = {
            "userId": "test_user",
            "front": "test front"
            # Missing 'back' and 'hint'
        }
        ...
        assert response.status_code == 400
        response_data = json.loads(response.data)
        assert response_data['message'] == "All fields must be provided"

        mock_db.child.assert_not_called()


    @patch('src.deck.routes.db')
    def test_record_wrong_answer_card_not_found(self, mock_db):
        '''Test the /deck/<deck_id>/record-wrong-answer route when card is not found'''
        deck_id = "test_deck_456"
        mock_query_result = MagicMock()
        mock_query_result.each.return_value = []
        mock_db.child.return_value.order_by_child.return_value.equal_to.return_value.get.return_value = mock_query_result
        ...
        response_data = json.loads(response.data)
        assert response_data['message'] == "Card not found in this deck"

        mock_db.child.assert_called_with("card")
        assert not mock_db.child().child().child().child().update.called
