...
class CardTestApp(unittest.TestCase):
    ...
    @patch("src.deck.routes.db")
    @patch("src.cards.routes.db")
    def test_getcards_success(self, mock_db):
        """Test fetching all cards in a deck successfully"""
        mock_card = MagicMock()
        ...
        mock_db.child.return_value.order_by_child.return_value.equal_to.return_value.get.return_value.each.return_value = [mock_card]
        response = self.client.get("/deck/Test/card/all")
        self.assertEqual(response.status_code, 200)
        data = response.get_json()
        self.assertEqual(data["cards"], [{"front": "frontVal", "back": "backVal", "hint": "hintVal"}])
        self.assertEqual(data["message"], "Cards fetched successfully")
    @patch("src.auth.routes.auth")
    @patch("src.deck.routes.db")
    @patch("src.cards.routes.db")
    def test_getcards_error(self, mock_db):
        """Test error handling in fetching all cards in a deck"""
        # Mock database to raise an exception
        mock_db.child.return_value.order_by_child.return_value.equal_to.return_value.get.return_value.each.side_effect = Exception("Database error")
        response = self.client.get("/deck/Test/card/all")
        self.assertEqual(response.status_code, 400)
        data = response.get_json()
        self.assertEqual(data["cards"], [])
        self.assertEqual(data["message"], "Failed to fetch cards")
